		...
		static void LoadPaths()
		{
			if (File.Exists("asset_paths.cfg"))
			{
				...
				foreach (var path in paths)
				{
					...
					var after_game_id = path.IndexOf(' ', "set ".Length);
					var id = path.Substring("set".Length + 1 + 5, after_game_id - ("set".Length + 1 + 5));
					var game = Globals.GetGameInfo(id);

					if (game == null)
						continue;

					game.Path = path.Substring(after_game_id + 2, path.Length - 1 - (after_game_id + 2));
				}
			}

			restart:
			for (var i = PaletteID.Q2; i < PaletteID.Total; i++)
			...
			if (WaitForYN())
			{
				using (var file = new StreamWriter("asset_paths.cfg", false))
				{
					file.WriteLine("// generated by transpiler, do not edit");

					foreach (var game in Globals.GameInfos)
						file.WriteLine("set {0} \"{1}\"", "path_" + game.ID, game.Path);
				}
				return;
			}
