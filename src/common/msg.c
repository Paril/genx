	...
	msgEsFlags_t          flags)
{
	...
	if (to->clip_contents != from->clip_contents)
		bits |= U_CLIPCONTENTS;

	if (to->renderfx & RF_FRAMELERP)
		bits |= U_OLDORIGIN;
	else if (to->renderfx & RF_PROJECTILE)
	{
	...
	// Paril
	if (bits & U_GAME)
		MSG_WriteByte(to->game);

	if (bits & U_CLIPCONTENTS)
...
void MSG_PackPlayer(player_packed_t *out, const player_state_t *in)
{
	...
	for (int i = 0; i < 3; ++i)
	{
		out->viewangles[i] = ANGLE2SHORT(in->viewangles[i]);
		out->viewoffset[i] = OFFSET2CHAR(in->viewoffset[i]);
		out->kick_angles[i] = OFFSET2CHAR(in->kick_angles[i]);
	}
	...
	for (int i = 0; i < MAX_PLAYER_GUNS; ++i)
	{
		...
		for (int x = 0; x < 3; ++x)
		{
			out->guns[i].angles[x] = OFFSET2CHAR(in->guns[i].angles[x]);
			out->guns[i].offset[x] = OFFSET2CHAR(in->guns[i].offset[x]);
		}
	}
	...
	msgPsFlags_t       flags)
{
	...
	if (!(flags & MSG_PS_IGNORE_PREDICTION))
	{
		...
		if (to->pmove.pm_time != from->pmove.pm_time ||
			to->pmove.pm_time2 != from->pmove.pm_time2)
			pflags |= PS_M_TIME;
	...
	{
		if (memcmp(to->guns, from->guns, sizeof(to->guns)) != 0)
			pflags |= PS_GUNS;
	}
	else
	{
		// save previous state
		memcpy(to->guns, from->guns, sizeof(to->guns));
	}
	...
	if (pflags & PS_M_TIME)
	{
		...
		MSG_WriteByte(to->pmove.pm_time2);
	}
	...
	if (pflags & PS_GUNS)
	{
		...
		for (i = 0; i < MAX_PLAYER_GUNS; ++i)
		{
			...
			if (!(flags & MSG_PS_IGNORE_GUNINDEX))
			{
				if (to->guns[i].index != from->guns[i].index)
					changed_bits |= PS_GUN_INDEX;
			}
			else
				to->guns[i].index = from->guns[i].index;

			if (!(flags & MSG_PS_IGNORE_GUNFRAMES))
			{
				if (to->guns[i].frame != from->guns[i].frame)
					changed_bits |= PS_GUN_FRAME;

				if (to->guns[i].offset[0] != from->guns[i].offset[0] ||
					to->guns[i].offset[1] != from->guns[i].offset[1] ||
					to->guns[i].offset[2] != from->guns[i].offset[2])
					changed_bits |= PS_GUN_OFFSET;

				if (to->guns[i].angles[0] != from->guns[i].angles[0] ||
					to->guns[i].angles[1] != from->guns[i].angles[1] ||
					to->guns[i].angles[2] != from->guns[i].angles[2])
					changed_bits |= PS_GUN_ANGLE;
			}
			else
			{
	...
	msgEsFlags_t   flags)
{
	...
	// Paril
	if (bits & U_GAME)
		to->game = (gametype_t)MSG_ReadByte();

	if (bits & U_CLIPCONTENTS)
		to->clip_contents = MSG_ReadLong();
}
	...
	int               extraflags)
{
	...
	if (flags & PS_M_TIME)
	{
		...
		to->pmove.pm_time2 = MSG_ReadByte();
	}
	...
	if (flags & PS_GUNS)
	{
		while (true)
		{
			byte gun = MSG_ReadByte();

			if (!gun)
				break;

			byte gun_index = gun & 0b00001111;
			...
			if (gun_flags & PS_GUN_OFFSET)
				for (int i = 0; i < 3; ++i)
					to->guns[gun_index].offset[i] = MSG_ReadChar() * 0.25f;

			if (gun_flags & PS_GUN_ANGLE)
				for (int i = 0; i < 3; ++i)
					to->guns[gun_index].angles[i] = MSG_ReadChar() * 0.25f;
		}
	}
