...
void HQ2x_Render(uint32_t *output, const uint32_t *input, int width, int height)
{
	...
	for (y = 0; y < height; y++)
	{
		...
		for (x = 0; x < width; x++)
		{
			...
			pattern |= diff(E, H) << 6;
			pattern |= diff(E, I) << 7;
			*(out0 + 0) = hq2x_blend(hqTable[pattern], E, A, B, D, F, H);
			pattern = rotTable[pattern];
			*(out0 + 1) = hq2x_blend(hqTable[pattern], E, C, F, B, H, D);
			pattern = rotTable[pattern];
			*(out1 + 1) = hq2x_blend(hqTable[pattern], E, I, H, F, D, B);
			pattern = rotTable[pattern];
...
void HQ4x_Render(uint32_t *output, const uint32_t *input, int width, int height)
{
	...
	for (y = 0; y < height; y++)
	{
		...
		for (x = 0; x < width; x++)
		{
			...
			pattern |= diff(E, H) << 6;
			pattern |= diff(E, I) << 7;
			hq4x_blend(hqTable[pattern], out0 + 0, out0 + 1, out1 + 0, out1 + 1, E, A, B, D, F, H);
			pattern = rotTable[pattern];
			hq4x_blend(hqTable[pattern], out0 + 3, out1 + 3, out0 + 2, out1 + 2, E, C, F, B, H, D);
			pattern = rotTable[pattern];
			hq4x_blend(hqTable[pattern], out3 + 3, out3 + 2, out2 + 3, out2 + 2, E, I, H, F, D, B);
			pattern = rotTable[pattern];
