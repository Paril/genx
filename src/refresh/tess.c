...
void GL_Flush2D(void)
{
	...
	else if (tess.flags & 4)
		bits |= GLS_BLEND_INVERT;
	...
	glDrawElements(GL_TRIANGLES, tess.numindices, GL_INDEX_ENUM, tess.indices);
...
void GL_DrawParticles(void)
{
	...
	// Generations
	for (int batch_type = 0; batch_type <= PARTICLE_OLDSCHOOL; ++batch_type)
	{
		p = glr.fd.particles;
		total = glr.fd.num_particles;
		// Generations
		GL_BindTexture(0, batch_type == PARTICLE_OLDSCHOOL ? TEXNUM_PARTICLE_Q1 : TEXNUM_PARTICLE);
		...
		do
		{
			...
			do
			{
				...
				{
					// Generations
					color.u32 = d_palettes[GAME_Q2][p->color & 0xff];
				...
				dst_vert[ 4] = 0;
				dst_vert[ 8] = 0;
				dst_vert[ 9] = PARTICLE_SIZE;
				dst_vert[13] = PARTICLE_SIZE;
				dst_vert[14] = 0;
...
void GL_DrawBeams(void)
{
	...
	for (i = 0, ent = glr.fd.entities; i < glr.fd.num_entities; i++, ent++)
	{
		...
		{
			// Generations
			color.u32 = d_palettes[GAME_Q2][ent->skinnum & 0xff];
...
void GL_BindArrays(void)
{
	...
	{
		ptr = NULL;
		glBindBuffer(GL_ARRAY_BUFFER, gl_static.world.bufnum);
	}
...
static int GL_TextureAnimation(mtexinfo_t *tex)
{
	...
	{
		ch = glr.ent->frame % tex->numframes;
