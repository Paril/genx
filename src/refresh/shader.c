...
{
	GLSL("layout(std140) uniform u_block {");
	GLSL("mat4 m_view;");
	GLSL("mat4 m_proj;");
	GLSL("float u_time;");
...
static GLuint create_and_use_program(GLbitfield bits)
{
	...
	glBindAttribLocation(program, VERT_ATTR_POS, "a_pos");
	glBindAttribLocation(program, VERT_ATTR_TC, "a_tc");

	if (bits & GLS_LIGHTMAP_ENABLE)
		glBindAttribLocation(program, VERT_ATTR_LMTC, "a_lmtc");

	if (!(bits & GLS_TEXTURE_REPLACE))
		glBindAttribLocation(program, VERT_ATTR_COLOR, "a_color");
	...
	GLuint index = glGetUniformBlockIndex(program, "u_block");
	...
	if (bits & GLS_LIGHTMAP_ENABLE)
		glUniform1i(glGetUniformLocation(program, "u_lightmap"), 1);
...
static void shader_state_bits(GLbitfield bits)
{
	...
	if (diff & GLS_SHADER_MASK)
	{
		...
		if (gl_static.programs[i])
			glUseProgram(gl_static.programs[i]);
		else
...
static void shader_array_bits(GLbitfield bits)
{
	...
	if (diff & GLA_VERTEX)
	{
		if (bits & GLA_VERTEX)
			...
			glDisableVertexAttribArray(VERT_ATTR_POS);
	}

	if (diff & GLA_TC)
	{
		if (bits & GLA_TC)
			glEnableVertexAttribArray(VERT_ATTR_TC);
		else
			glDisableVertexAttribArray(VERT_ATTR_TC);
	}

	if (diff & GLA_LMTC)
	{
		if (bits & GLA_LMTC)
			glEnableVertexAttribArray(VERT_ATTR_LMTC);
		else
			glDisableVertexAttribArray(VERT_ATTR_LMTC);
	}
	...
	if (diff & GLA_COLOR)
	{
		if (bits & GLA_COLOR)
			...
			glDisableVertexAttribArray(VERT_ATTR_COLOR);
	}
}

static void shader_vertex_pointer(GLint size, GLsizei stride, const GLfloat *pointer)
{
	glVertexAttribPointer(VERT_ATTR_POS, size, GL_FLOAT, GL_FALSE, sizeof(GLfloat) * stride, pointer);
}

static void shader_tex_coord_pointer(GLint size, GLsizei stride, const GLfloat *pointer)
{
	glVertexAttribPointer(VERT_ATTR_TC, size, GL_FLOAT, GL_FALSE, sizeof(GLfloat) * stride, pointer);
}

static void shader_light_coord_pointer(GLint size, GLsizei stride, const GLfloat *pointer)
{
	glVertexAttribPointer(VERT_ATTR_LMTC, size, GL_FLOAT, GL_FALSE, sizeof(GLfloat) * stride, pointer);
}

static void shader_color_byte_pointer(GLint size, GLsizei stride, const GLubyte *pointer)
{
	glVertexAttribPointer(VERT_ATTR_COLOR, size, GL_UNSIGNED_BYTE, GL_TRUE, sizeof(GLfloat) * stride, pointer);
}

static void shader_color_float_pointer(GLint size, GLsizei stride, const GLfloat *pointer)
{
	glVertexAttribPointer(VERT_ATTR_COLOR, size, GL_FLOAT, GL_FALSE, sizeof(GLfloat) * stride, pointer);
}

static void shader_color(GLfloat r, GLfloat g, GLfloat b, GLfloat a)
{
	glVertexAttrib4f(VERT_ATTR_COLOR, r, g, b, a);
}

static void upload_u_block(void)
{
	glBufferData(GL_UNIFORM_BUFFER, sizeof(gls.u_block), &gls.u_block, GL_DYNAMIC_DRAW);
}
...
static void shader_clear(void)
{
	...
	if (gl_static.programs[0])
		glUseProgram(gl_static.programs[0]);
	else
...
{
	glGenBuffers(1, &gl_static.u_bufnum);
	glBindBuffer(GL_UNIFORM_BUFFER, gl_static.u_bufnum);
	glBindBufferBase(GL_UNIFORM_BUFFER, 0, gl_static.u_bufnum);
	glBufferData(GL_UNIFORM_BUFFER, sizeof(gls.u_block), NULL, GL_DYNAMIC_DRAW);
}
...
static void shader_shutdown(void)
{
	...
	for (int i = 0; i < MAX_PROGRAMS; i++)
	{
		if (gl_static.programs[i])
		{
			glDeleteProgram(gl_static.programs[i]);
	...
	{
		glDeleteBuffers(1, &gl_static.u_bufnum);
