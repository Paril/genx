...
void doom_caco_ball_touch(edict_t *ent, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	MSG_WriteByte(TE_DOOM_CACO_BOOM);
	MSG_WritePos(origin);
...
void fire_doom_caco_ball(edict_t *self, vec3_t start, vec3_t dir, int damage, int speed)
{
	...
	rocket->server.state.modelindex = gi.modelindex("sprites/doom/BAL2.d2s");
...
void doom_monster_head(edict_t *self)
{
	...
	self->server.state.modelindex = gi.modelindex("sprites/doom/head.d2s");
	self->health = 400;
	self->dmg = 0;
