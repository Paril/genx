...
{
	gib->s.frame = (gib->s.frame + 1) % (gib->count == GIB_DUKE_LG ? 8 : gib->count == GIB_DUKE_SM ? 2 : 4);
...
void ThrowHead(edict_t *self, char *gibname, int damage, gibtype_e type)
{
	...
	self->s.renderfx &= ~(RF_NOLERP | RF_FRAMELERP);
	self->s.sound = 0;
