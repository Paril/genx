...
static bool AI_PredictJumpadDestity(edict_t *ent, vec3_t out)
{
	...
	if ((trace.fraction == 1.0f && trace.startsolid) || (trace.allsolid && trace.startsolid))
	{
...
//==========================================
static int AI_AddNode_BotRoam(edict_t *ent)
{
	if (nav.num_nodes + 1 > MAX_NODES)
		return INVALID;
	...
	if (ent->spawnflags & 1)   // floating items
		nodes[nav.num_nodes].flags |= NODEFLAGS_FLOAT;
	else if (!AI_DropNodeOriginToFloor(nodes[nav.num_nodes].origin, NULL))
	...
	if (ent->count)
		nav.broams[nav.num_broams].weight = ent->count * 0.01f;//count is a int with a value in between 0 and 100
	else
...
//==========================================
static int AI_AddNode_ItemNode(edict_t *ent)
{
	if (nav.num_nodes + 1 > MAX_NODES)
		return INVALID;
...
//==========================================
static void AI_CreateNodesForEntities(void)
{
	edict_t *ent;
	int		node;
	nav.num_ents = 0;
				...
				!(nodes[node].flags & NODEFLAGS_LADDER))
			{
				...
				if (heightdiff < AI_STEPSIZE && nav.num_broams < MAX_BOT_ROAMS)  //near enough
				{
					...
					if (ent->count)
						...
						nav.broams[nav.num_broams].weight = 0.3f; //jalfixme: add cmd to weight (dropped by console cmd, self is player)
