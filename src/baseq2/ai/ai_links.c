...
static edict_t		*LINKS_PASSENT = NULL;
...
//==========================================
static float AI_FindLinkDistance(int n1, int n2)
{
...
static int AI_GravityBoxStep( vec3_t origin, float scale, vec3_t destvec, vec3_t neworigin, vec3_t mins, vec3_t maxs )
{
	...
	{	
		...
		VectorMA( origin, xzscale, forward, neworigin );
		trace = gi.trace( origin, mins, maxs, neworigin, LINKS_PASSENT, MASK_NODESOLID );
		if( trace.plane.normal[2] < 0.5f && trace.plane.normal[2] >= -0.4f )
		{
...
static int AI_IsLadderLink( int n1, int n2 )
{
	...
	if( nodes[n1].flags & NODEFLAGS_LADDER && !(nodes[n2].flags & NODEFLAGS_LADDER) )
	{
		...
		if( AI_Distance(nodes[n1].origin, nodes[n2].origin) > (NODE_DENSITY*0.8f) )
			return LINK_INVALID;
