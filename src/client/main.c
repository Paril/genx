...
const centity_t nullEntity =
{
	.current = {
		.clip_contents = CONTENTS_MONSTER
	},
	.prev = {
		.clip_contents = CONTENTS_MONSTER
	}
...
void CL_ClearState(void)
{
	...
	for (int i = 0; i < MAX_EDICTS; ++i)
		memcpy(&cl_entities[i], &nullEntity, sizeof(nullEntity));
...
{
	dump_program(cl.layout_raw, "layout");
}
...
===============
*/
static void CL_WriteConfig(char *dir, char *filename, bool aliases, bool bindings, bool modified, int cvar_mask)
{
	char buffer[MAX_OSPATH];
	qhandle_t f;
	...
	if (!aliases && !bindings && !cvar_mask)
	{
		bindings = true;
		cvar_mask = CVAR_ARCHIVE;
	}
	...
	if (cvar_mask)
	{
		...
		Cvar_WriteVariables(f, cvar_mask, modified);
	}
...
static void CL_WriteConfig_f(void)
{
	...
	CL_WriteConfig("configs/", cmd_optarg, aliases, bindings, modified, mask);
}
...
void CL_RestartFilesystem(bool total)
{
	...
	CL_WriteConfig("", COM_CONFIG_CFG, false, true, true, CVAR_ARCHIVE);
...
gametype_t CL_GetClientGame()
{
	...
	return cl_entities[cl.frame.clientNum + 1].current.game;
}